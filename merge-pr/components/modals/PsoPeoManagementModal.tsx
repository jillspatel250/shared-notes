...
import { toast } from "sonner"
import {
  fetchDepartmentPsoPeo,
  saveDepartmentPsoPeo,
  getCurrentUserDepartmentId,
  type PSOPEOItem,
} from "@/app/dashboard/actions/managePsoPeo"
...
export default function PsoPeoManagementModal({ isOpen, onClose }: PsoPeoManagementModalProps) {
  const [loading, setLoading] = useState(false)
  const [saving, setSaving] = useState(false)
  const [psoItems, setPsoItems] = useState<PSOPEOItem[]>([{ id: "pso1", label: "PSO1", description: "" }])
  const [peoItems, setPeoItems] = useState<PSOPEOItem[]>([{ id: "peo1", label: "PEO1", description: "" }])
  ...
  const loadDepartmentInfo = async () => {
    setLoading(true)
    try {
      const result = await getCurrentUserDepartmentId()

      if (!result.success) {
        toast.error(result.error || "Failed to load department information")
        return
      }
  ...
  const loadDepartmentPsoPeoData = async () => {
    if (!departmentId) return

    try {
      const result = await fetchDepartmentPsoPeo(departmentId)

      if (!result.success) {
        console.error("Error loading PSO/PEO data:", result.error)
        return
      }

      if (result.data) {
        // Load PSO data
        if (result.data.pso_data && result.data.pso_data.length > 0) {
          setPsoItems(result.data.pso_data)
        }

        // Load PEO data
        if (result.data.peo_data && result.data.peo_data.length > 0) {
          setPeoItems(result.data.peo_data)
        }
      }
    } catch (error) {
      console.error("Error loading PSO/PEO data:", error)
  ...
  // Save PSO/PEO data to department_pso_peo table
  const savePsoPeoData = async () => {
    if (!departmentId) {
      toast.error("Department information not found")
      return
    ...
    setSaving(true)
    try {
      const result = await saveDepartmentPsoPeo(departmentId, psoItems, peoItems)

      if (!result.success) {
        toast.error(result.error || "Failed to save PSO/PEO data")
        return
      }

      toast.success("PSO/PEO data saved successfully for the department")
      onClose() // Close modal after successful save
    } catch (error) {
      console.error("Error saving PSO/PEO data:", error)
      toast.error("Failed to save PSO/PEO data")
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle className="text-xl font-bold">PSO/PEO Management</DialogTitle>
          <p className="text-sm text-gray-600">Manage PSO/PEO for {departmentName} Department</p>
        </DialogHeader>
