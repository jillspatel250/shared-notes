...
import { saveUnitPlanningForm } from "@/app/dashboard/actions/saveUnitPlanningForm"
import { loadSharedUnitPlanningData } from "@/app/dashboard/actions/loadSharedUnitPlanningData"
import { useDashboardContext } from "@/context/DashboardContext"
...
export default function UnitPlanningForm({ lessonPlan, setLessonPlan }: UnitPlanningFormProps) {
  ...
  // LOAD SHARED UNIT PLANNING DATA
  useEffect(() => {
    const loadUnitPlanningData = async () => {
      if (!lessonPlan?.subject?.id || !userData?.id) {
        console.log("❌ Missing data for unit planning loading")
        return
      }

      console.log("🔄 Loading shared unit planning data...")

      try {
        const result = await loadSharedUnitPlanningData(lessonPlan.subject.id, userData.id)
        console.log("📋 Shared unit planning result:", result)
        ...
        if (result.success) {
          ...
          if (result.formData && result.formData.form) {
            const formData = result.formData.form
            console.log("📊 Loading unit planning data:", formData)

            if (formData.unitPlanning) {
              const up = formData.unitPlanning
              console.log("📋 Unit Planning Data:", up)

              setUnits(up.units || [])
              setRemarks(up.remarks || "")

              // Set last updated info
              if (up.last_updated_by) {
                const updatedByFaculty = result.allFaculty.find((f) => f.user_id === up.last_updated_by)
                setLastUpdatedBy(updatedByFaculty?.users?.name || "Unknown Faculty")
              }

              console.log("✅ Unit planning data loaded successfully")
              toast.success("Shared unit planning data loaded!")
            }
          }
        }
      } catch (error) {
        console.error("💥 Error loading shared unit planning data:", error)
        toast.error("Failed to load unit planning data")
      }
    }

    loadUnitPlanningData()
  }, [lessonPlan?.subject?.id, userData?.id])
  ...
  const handleCheckboxChange = (unitIndex: number, field: string, value: string, checked: boolean) => {
    const updatedUnits = [...units]
    const currentValues = updatedUnits[unitIndex][field] || []

    if (checked) {
      updatedUnits[unitIndex][field] = [...currentValues, value]
    } else {
      updatedUnits[unitIndex][field] = currentValues.filter((item: string) => item !== value)
    }
 ...
 `
  ...
  // Replace the validateDateOrder function with this simpler version
  const validateDateOrder = (unitIndex: number) => {
    const startDate = getValues(`units.${unitIndex}.probable_start_date`)
    const endDate = getValues(`units.${unitIndex}.probable_end_date`)
      ...
      const formData = {
        ...
        formData: {
          units: units.map((unit) => ({
            ...unit,
            // Add "Other:" prefix to custom pedagogies
            teaching_pedagogy: unit.teaching_pedagogy
              .map((pedagogy) =>
                pedagogy === "Other" && unit.other_pedagogy ? `Other: ${unit.other_pedagogy}` : pedagogy,
              )
              .filter((p) => p !== "Other" || unit.other_pedagogy),
          })),
          remarks,
        },
      }

      console.log("📤 Submitting unit planning data:", formData)

      const result = await saveUnitPlanningForm(formData)

      if (result.success) {
        setLessonPlan((prev: any) => ({
          ...prev,
          units,
          unit_remarks: remarks,
          unit_planning_completed: true,
          status: "in_progress",
        }))

        toast.success(result.message || "Unit planning saved successfully!")

        setTimeout(() => {
          const nextTab = document.querySelector('[value="cie-planning"]')
          if (nextTab) {
 ...
 `
  ...
  // Generate CO options based on course outcomes
  const coOptions = lessonPlan?.courseOutcomes?.map((_: any, index: number) => `CO${index + 1}`)
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-yellow-600">
              Units: {units.length} | Shared: {isShared ? "Yes" : "No"} | Faculty Count: {allFaculty.length}
            </p>
          </div>
        </div>
      </div>

      {/* SHARED SUBJECT BANNER */}
      {isShared && (
        <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <Users className="h-5 w-5 text-blue-600" />
              <div>
                <h3 className="text-sm font-medium text-blue-800">Shared Subject - Unit Planning</h3>
                <p className="text-xs text-blue-600">
                  This unit planning is shared with {allFaculty.length} faculty member(s) teaching this subject
            ...
            >
              {showSharedInfo ? "Hide" : "View"} Faculty List
            </Button>
          </div>

          {showSharedInfo && (
            <div className="mt-4 p-3 bg-white border rounded">
              <h4 className="font-medium text-sm text-gray-800 mb-2">Faculty teaching this subject:</h4>
              <div className="space-y-1">
                {allFaculty.map((faculty, index) => (
                  <div key={faculty.user_id} className="text-xs text-gray-600">
                    {index + 1}. {faculty.users?.name || "Unknown"} ({faculty.users?.email || "No email"})
                    {faculty.user_id === userData?.id && <span className="text-blue-600 font-medium"> (You)</span>}
                  </div>
                ))}
              </div>
            </div>
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    // Remove the validateDateOrder call from here
                  }}
                />
                {errors.units?.[activeUnit]?.probable_start_date && (
