...
export default function PracticalPlanningForm({ lessonPlan, setLessonPlan, userData }: PracticalPlanningFormProps) {
  ...
  useEffect(() => {
    ...
    fetchTermDates()
  }, [lessonPlan?.subject?.code, userData?.id])
  ...
  const handleFacultyAssignment = (practicalIndex: number, facultyId: string) => {
    ...
    const updatedPracticals = [...(lessonPlan.practicals || [])]
    updatedPracticals[practicalIndex] = {
      ...updatedPracticals[practicalIndex],
  ...
  const handleSave = async () => {
    ...
    if (isSharing) {
      const unassignedPracticals = lessonPlan.practicals.filter((practical) => !practical.assigned_faculty_id)
      if (unassignedPracticals.length > 0) {
        const practicalNumbers = unassignedPracticals
          .map((_, idx) => {
            const originalIndex = lessonPlan.practicals.findIndex((p) => p.id === unassignedPracticals[idx].id)
            return originalIndex + 1
          })
          .join(", ")

        toast.error(`Please assign faculty to Practical ${practicalNumbers} before saving.`)
        setSaving(false)
        return
      }
    } else {
      // For non-shared subjects, automatically assign current faculty to all practicals
      lessonPlan.practicals.forEach((practical, index) => {
        if (!practical.assigned_faculty_id) {
          lessonPlan.practicals[index] = {
            ...practical,
      ...
      {isSharing && (
        <div className="bg-green-50 border border-green-200 rounded-lg p-3 mb-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-2">
            {currentPracticals.map((practical, index) => {
